...
char *pushQCommaSepFieldNames = "qid,pqid,priority,rank,qadate,newYN,track,dbs,tbls,cgis,files,sizeMB,currLoc,makeDocYN,onlineHelp,ndxYN,joinerYN,stat,sponsor,reviewer,extSource,openIssues,notes,pushState,initdate,lastdate,bounces,lockUser,lockDateTime,releaseLog,featureBits,releaseLogUrl,importance";
...
safecpy(ret->importance, sizeof(ret->importance), row[32]);
}
...
sqlDyStringPrintf(update, "insert into %s values ( '%s','%s','%s',%u,'%s','%s','%s','%s','%s','%s','%s',%u,'%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s',%u,'%s','%s','%s','%s','%s','%s')", 
	tableName,  el->qid,  el->pqid,  el->priority,  el->rank,  el->qadate,  el->newYN,  el->track,  el->dbs,  el->tbls,  el->cgis,  el->files,  el->sizeMB,  el->currLoc,  el->makeDocYN,  el->onlineHelp,  el->ndxYN,  el->joinerYN,  el->stat,  el->sponsor,  el->reviewer,  el->extSource,  el->openIssues,  el->notes,  el->pushState,  el->initdate,  el->lastdate,  el->bounces,  el->lockUser,  el->lockDateTime,  el->releaseLog,  el->featureBits,  el->releaseLogUrl,  el->importance);
...
sqlFixedStringComma(&s, ret->importance, sizeof(ret->importance));
*pS = s;
return ret;
}
...
fputc(sep,f);
if (sep == ',') fputc('"',f);
fprintf(f, "%s", el->importance);
if (sep == ',') fputc('"',f);
fputc(lastSep,f);
}


char *usersCommaSepFieldNames = "user,password,role,contents";
